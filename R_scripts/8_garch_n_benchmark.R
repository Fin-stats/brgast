...
map_garch <- function(raw) {
  w <- exp(raw[3:4])
  alpha <- 0.995 * w[1] / (1 + sum(w))
  beta <- 0.995 * w[2] / (1 + sum(w))
  list(mu = raw[1], omega = exp(raw[2]), alpha = alpha, beta = beta)
}

filter_garch <- function(y, params) {
  e <- y - params$mu
  n <- length(y)
  sig2 <- rep(max(stats::var(y), 1e-6), n)
  ll <- rep(NA_real_, n)
  for (i in seq_len(n)) {
    ll[i] <- stats::dnorm(e[i] / sqrt(sig2[i]), log = TRUE) - log(sqrt(sig2[i]))
    if (i < n) {
...
fit_garch_n <- function(y, start = NULL) {
  if (is.null(start)) {
    start <- c(mean(y), log(0.05 * stats::var(y)), stats::qlogis(0.05), stats::qlogis(0.90))
  }
  obj <- function(raw) -filter_garch(y, map_garch(raw))$llk
  ...
  params <- map_garch(fit$par)
  filt <- filter_garch(y, params)
  list(params = params, raw = fit$par, next_sig2 = filt$next_sig2)
}

forecast_garch_n <- function(fit, tau) {
  sig <- sqrt(fit$next_sig2)
  data.frame(
    tau = tau,
    var = fit$params$mu + sig * stats::qnorm(tau),
    es = fit$params$mu - sig * stats::dnorm(stats::qnorm(tau)) / tau,
...
for (j in seq_along(oos_idx)) {
  ...
  if (j == 1L || ((j - 1L) %% refit_every) == 0L || is.null(current_fit)) {
    ...
    current_fit <- fit_garch_n(train, current_start)
    current_start <- current_fit$raw
  }
  fc <- forecast_garch_n(current_fit, tau)
...
garch_n_forecasts <- do.call(rbind, rows)

write.csv(garch_n_forecasts, file.path(folder_path, "garch_n_benchmark_forecasts.csv"), row.names = FALSE)

print(head(garch_n_forecasts))
